...
public class CommandHandler {
    ...
    private boolean banrichtlijn() {
        ...
        sender.sendMessage(ColorFormatter.replaceColors("&3Ongepast taalgebruik &6- &2Waarschuwing (kick), daarna 2 tot 4 uur ban"));
        sender.sendMessage(ColorFormatter.replaceColors("&3Spam &6- &2Waarschuwing (kick), daarna 2 tot 4 uur ban"));
        sender.sendMessage(ColorFormatter.replaceColors("   "));
        sender.sendMessage(ColorFormatter.replaceColors("&eBedenk je ban verstandig en zet er een DUIDELIJKE reden bij."));
        sender.sendMessage(ColorFormatter.replaceColors("&7Mocht je dit niet kunnen, geef dit door aan je leidinggevende!"));
        return true;
    }
        ...
        if (sender.hasPermission("iMine.mchistory")) {
            if (args.length > 0) {
                ...
                for (final UUID foundUUID : uuidsLike) {
                    Bukkit.getScheduler().runTaskAsynchronously(BukkitStarter.plugin, new NameLookup(foundUUID, sender));
                }
            } else {
    ...
    private boolean plugin() {
        ...
        for (Plugin plugin : plugins) {
            ...
            try {
                ...
                extra.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder(plugin.getDescription().getVersion()).append("\n\n" + f.getName()).create()));
            } catch (Exception ex) {
                ex.printStackTrace();
            }
            message.addExtra(extra);
    ...
    private boolean mute() {
        if (args.length != 0 && sender.hasPermission("iMine.mute")) {
            ...
            if (pl != null) {
                ...
                sender.sendMessage(ChatColor.GOLD + pl.getName() + " is now " + (BukkitListener.isMuted(pl) ? "muted." : "unmuted."));
            } else {
        ...
        if (sender.hasPermission("iMine.lagdebug")) {
            if (args.length == 0) {
                for (World w : Bukkit.getWorlds()) {
                    sender.sendMessage(ChatColor.GOLD + "Mobs in world " + w.getName() + " " + ChatColor.BOLD + w.getEntities().size());
                    ...
                    for (Class<? extends Entity> entityClass : MapCountSorter.getOrder(countMap, Sort.DESC)) {
                        sender.sendMessage(ChatColor.GREEN + "  " + entityClass.getSimpleName().replace("Craft", "") + "s: " + ChatColor.BOLD + countMap.get(entityClass).size());
                    }
                }
            } else {
                if (args[0].toLowerCase().startsWith("player")) {
                    ...
                    for (String plName : MapCountSorter.getOrder(countMap, Sort.DESC)) {
                        sender.sendMessage(ChatColor.AQUA + "  " + plName + ": " + ChatColor.BOLD + countMap.get(plName).size());
                    }
                } else {
                    Map<Class<? extends Entity>, List<Entity>> countMap = new HashMap<>();
            ...
            if (args.length == 1) {
                if (sender instanceof Player) {
                    ...
                    if (pl != null) {
                        ...
                        sender.sendMessage(ChatColor.GOLD + "Teleporting to " + ChatColor.RED + pl.getName() + ChatColor.GOLD + ".");
                    } else {
                ...
                if (who != null) {
                    if (target != null) {
                        ...
                        sender.sendMessage(ChatColor.GOLD + "Teleported " + ChatColor.RED + who.getName() + ChatColor.GOLD + " to " + ChatColor.RED + target.getName() + ChatColor.GOLD + ".");
                    } else {
                ...
                try {
                    if (faultArg == 0 && args.length == 4) {
                        ...
                        if (target != null) {
                            target.teleport(new Location(target.getWorld(), coords[1], coords[2], coords[3], target.getLocation().getYaw(), target.getLocation().getPitch()));
                        ...
                        if (sender instanceof Player) {
                            ...
                            if (args.length == 3) {
                                who.teleport(new Location(who.getWorld(), coords[0], coords[1], coords[2], who.getLocation().getYaw(), who.getLocation().getPitch()));
                                sender.sendMessage(ChatColor.GOLD + "Teleporting...");
                            } else {
                                if (Bukkit.getWorlds().get((int) coords[3]) != null) {
                                    who.teleport(new Location(Bukkit.getWorlds().get((int) coords[3]), coords[0], coords[1], coords[2], who.getLocation().getYaw(), who.getLocation().getPitch()));
                                    sender.sendMessage(ChatColor.GOLD + "Teleporting...");
                                } else {
                ...
                if (args.length == 1) {
                    ...
                    if (who != null) {
                        if (set != null) {
                            ...
                            sender.sendMessage(ChatColor.GOLD + "Set gamemode to " + ChatColor.RED + set.name().toLowerCase() + ChatColor.GOLD + " for " + ChatColor.RED + who.getName() + ChatColor.GOLD + ".");
                        } else {
                            sender.sendMessage(ChatColor.RED + "No gamemode found by that name.");
                        }
                    } else {
        ...
        if (sender.hasPermission("iMine.msg")) {
            if (args.length > 1) {
                ...
                if (target != null) {
                    ...
                    msg = ColorFormatter.replaceColors(msg);
                    target.sendMessage(ChatColor.DARK_GRAY.toString() + ChatColor.ITALIC + "Received message from " + ChatColor.RED + sender.getName() + ChatColor.DARK_GRAY + ChatColor.BOLD + " \u00BB " + ChatColor.RESET + ChatColor.GRAY + msg);
                    sender.sendMessage(ChatColor.DARK_GRAY.toString() + ChatColor.ITALIC + "Send message to " + ChatColor.RED + target.getName() + ChatColor.DARK_GRAY + ChatColor.BOLD + " \u00BB " + ChatColor.RESET + ChatColor.GRAY + msg);
    ...
    private boolean git() {
        ...
        sender.sendMessage(String.format("%s%s[%s%sGIT%s%s]%s Checking all git repos...", ChatColor.RESET, ChatColor.BOLD, ChatColor.GOLD, ChatColor.BOLD, ChatColor.RESET, ChatColor.BOLD, ChatColor.RESET));
    ...
    private boolean me() {
        if (sender.hasPermission("iMine.me")) {
            if (args.length > 0) {
                ...
                msg = msg.substring(0, msg.length() - 1);
                msg = ColorFormatter.replaceColors(msg);
                Bukkit.broadcastMessage(ChatColor.GOLD + "* " + ChatColor.GRAY + sender.getName() + " " + ChatColor.WHITE + msg);
            } else {
                sender.sendMessage(ChatColor.RED + "Need a message to tell.");
            }
        } else {
    ...
    public static List<String> onTabComplete(Player sender, String command, String[] args) {
        ...
        } else if ((command.startsWith("gm") && command.length() == 3) || (command.equalsIgnoreCase("gm") && args.length == 2) || (command.equalsIgnoreCase("mute") && args.length == 1) || (command.equalsIgnoreCase("fly") && args.length == 1) || (command.equalsIgnoreCase("invsee") && args.length == 1) || (command.equalsIgnoreCase("vanish") && args.length == 1) || (command.equalsIgnoreCase("report")) || (command.equalsIgnoreCase("tp") && (args.length == 1 || args.length == 2)) || (command.equalsIgnoreCase("kill") && args.length == 1) || (command.equalsIgnoreCase("reply")) || (command.equalsIgnoreCase("endersee") && args.length == 1) || (command.equalsIgnoreCase("msg")) || (command.equalsIgnoreCase("speed") && args.length > 1) || (command.equalsIgnoreCase("me"))) {
            ret.addAll(PlayerGetter.getAllOnlineNames(args[args.length - 1], sender));
        } else if (command.equalsIgnoreCase("update")) {
        } else if (command.equalsIgnoreCase("lagdebug")) {
    ...
    private static void globalAdminMessage(Player sender, String message) {
        ...
        ResultSet rs = db.doQuery("SELECT UUID_Table.LastName FROM AdminRegister JOIN UUID_Table " + "ON UUID_Table.UUID = AdminRegister.UUID;");
        try {
            while (rs.next()) {
        ...
        private void sendNameInfo(String name, long time) {
            ...
            if (time > 0L) {
                ...
                if (name == null) {
                    ...
                    sender.sendMessage(ColorFormatter.replaceColors("&6 Name: '&c" + name + "&6' changed " + MKTUtils.timeUntilNow(d) + " ago."));
                }
            } else {
                sender.sendMessage(ColorFormatter.replaceColors("&6Getting al old playernames from '&c" + name + "&6'."));
                return;
            }
        }
    }
    ...
    private static class ServerReporter implements Runnable {
        private static final String FORMAT_MESSAGE = String.format("%s%s[%s%sREPORT%s%s] %s%s%s %s\u00BB%s %s", ChatColor.RESET, ChatColor.BOLD, ChatColor.RED, ChatColor.BOLD, ChatColor.RESET, ChatColor.BOLD, ChatColor.RESET, ChatColor.GRAY, "%s", ChatColor.BOLD, ChatColor.RED, "%s");
    ...
    private static class AdminChat implements Runnable {
        ...
        private static final String FORMAT_MESSAGE = String.format("%s%s[%s%sADMIN%s%s] %s%s%s %s\u00BB%s %s", ChatColor.RESET, ChatColor.BOLD, ChatColor.GREEN, ChatColor.BOLD, ChatColor.RESET, ChatColor.BOLD, ChatColor.RESET, ChatColor.GRAY, "%s", ChatColor.BOLD, ChatColor.RESET, "%s");
    ...
    private static class ReloadPlugin implements Runnable {
        private static final String FORMAT = String.format("%s%s[%s%sUpdater%s%s]%s %s", ChatColor.RESET, ChatColor.BOLD, ChatColor.AQUA, ChatColor.BOLD, ChatColor.RESET, ChatColor.BOLD, ChatColor.RESET, "%s");
        ...
        public void run() {
            ...
            if (pl != null) {
                ...
                sender.sendMessage(String.format(FORMAT, pl.getName() + " is now disabled [" + pl.getDescription().getVersion() + "]"));
                try {
                    Thread.sleep(3000L);
                    Method m = JavaPlugin.class.getDeclaredMethod("getFile");
                    m.setAccessible(true);
                    ...
                    sender.sendMessage(String.format(FORMAT, pl.getName() + " is now reloaded! [" + pl.getDescription().getVersion() + "]"));
    ...
    private static class GitCheckRunnalbe implements Runnable {
        ...
        private void checkPlugin(Plugin plugin) {
            ...
            if (match.find()) {
                ...
                TextComponent extra, message = new TextComponent("");
                // [GIT]
                extra = new TextComponent(String.format("%s%s[%s%sGIT%s%s]%s ", ChatColor.RESET, ChatColor.BOLD, ChatColor.GOLD, ChatColor.BOLD, ChatColor.RESET, ChatColor.BOLD, ChatColor.RESET));
                message.addExtra(extra);
                // %plugin naam%
                ...
                extra.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder(git.getDescription()).append("\n\nCreated at: " + GitLabAPI.NL_DATE_FORMAT.format(git.getCreateDate())).create()));
                message.addExtra(extra);
                // current verion:
                ...
                extra = new TextComponent(String.format("%s%s%s ", ChatColor.GOLD, (current == null ? ChatColor.RED + "not found" : current.getTitle().replaceAll(" ", " " + ChatColor.GOLD)), ChatColor.RESET));
                extra.setColor(net.md_5.bungee.api.ChatColor.GOLD);
                extra.setClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, git.getWebUrl() + "/commit/" + (current == null ? "master" : current.getLongId()) + "?view=parallel"));
                extra.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder((current == null ? ChatColor.RED + match.group(0) + " - commit not found!" : "version id: " + current.getShortId())).append("\n\nPushed at: " + (current == null ? "~" : GitLabAPI.NL_DATE_FORMAT.format(current.getWhen()))).create()));
                message.addExtra(extra);
                if (commits.isEmpty()) {
                    ...
                    extra = new TextComponent(String.format("%s%s%s ", ChatColor.GOLD, git.getCommits()[0].getTitle().replaceAll(" ", " " + ChatColor.GOLD), ChatColor.RESET));
                    extra.setColor(net.md_5.bungee.api.ChatColor.GOLD);
                    extra.setClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, git.getWebUrl() + "/compare/" + (current == null ? "master" : current.getShortId()) + "...master?view=parallel"));
                    ComponentBuilder hoverBuilder = new ComponentBuilder("version id: " + git.getCommits()[0].getShortId());
                    hoverBuilder.append("\nMissing Versions:");
                    for (Commit commit : commits) {
                        hoverBuilder.append("\n" + ChatColor.GOLD + " " + ChatColor.GRAY + commit.getTitle() + "  [" + GitLabAPI.NL_DATE_FORMAT.format(commit.getWhen()) + "]");
                    }
                    ...
                    message.addExtra(extra);
                    // versions behind [#]:
                    extra = new TextComponent(String.format("%s[%d]%s ", ChatColor.RESET, commits.size(), ChatColor.RESET));
                    message.addExtra(extra);
                }
                if (quietSend()) {
        ...
        private void prosessData() {
            ...
            if (gitUpdates > 0 || updates > 0) {
                ...
                for (File newFile : BukkitStarter.UPDATE_DIR.listFiles()) {
                    ...
                    if (BukkitStarter.UPDATE_DIR.listFiles()[BukkitStarter.UPDATE_DIR.listFiles().length - 1] != newFile) {
                        append += "\n";
                    }
                    cb.append(append);
                }
                ...
                // [RELOAD SERVER]
                extra = new TextComponent(String.format(" %s%s[%sReload Server%s%s]%s ", ChatColor.RESET, ChatColor.BOLD, ChatColor.DARK_GREEN + (updates == 0 ? ChatColor.STRIKETHROUGH.toString() : ""), ChatColor.RESET, ChatColor.BOLD, ChatColor.RESET));
                extra.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/reload"));
                extra.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder("Click to reload server").create()));
                message.addExtra(extra);
                // [REBOOT SERVER]
                extra = new TextComponent(String.format("%s%s[%sReboot Server%s%s]", ChatColor.RESET, ChatColor.BOLD, ChatColor.RED + (updates == 0 ? ChatColor.STRIKETHROUGH.toString() : ""), ChatColor.RESET, ChatColor.BOLD));
                extra.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/restart"));
                extra.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder(ChatColor.RED + "WARNING, will shutdown server!\n" + ChatColor.RESET + "Click to reboot server").create()));
                message.addExtra(extra);
                sendTextComponent(message);
            } else {
